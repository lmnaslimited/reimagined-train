...
import { getTermsAndConditionsData } from "./api/termsAndCondition";

export default function TermsAndCondition({termsAndConditionPageData}:{termsAndConditionPageData:TtermsAndConditionData}) {
  ...
  return (
    ...
    <Layout ilayoutData={termsAndConditionPageData[0].layoutData}>
    <main id="content" role="main">
      <div className="md:max-w-screen-sm text-center px-4 sm:px-6 lg:px-8 pt-24 pb-6 mx-auto">
        <h1 className="text-2xl font-bold md:text-4xl ">
          {termsAndConditionPageData[0].title}
        </h1>
        <p className="text-md mt-4">
          {termsAndConditionPageData[0].subTitle.text1}
        </p>
        <p className="mt-2">
          {termsAndConditionPageData[0].subTitle.text2}
        </p>
      </div>

      <div className="md:max-w-screen-sm lg:max-w-[992px] px-4 sm:px-6 lg:px-8 pb-12 md:pt-6 sm:pb-20 mx-auto">
        <div className="grid gap-4 md:gap-8">
          {termsAndConditionPageData[0].tcContents.map((tcContent)=>(<div key={tcContent.content.text1}>
            <h2 className="text-lg sm:text-xl font-semibold mb-2 ">
              {tcContent.heading}
            </h2>
            <p className="mt-3 mb-5 ">
              {tcContent.content.text1}
            </p>

            <p className="mb-5">
              {tcContent.content.text2}
            </p>
            <p className="mb-5">
              {tcContent.content.text3}
            </p>
          </div>))}
          <div>
            <h2 className="text-lg sm:text-xl font-semibold mb-2 ">
              {termsAndConditionPageData[0].contact.heading}
            </h2>
            <p className="mt-3 mb-5">
              {termsAndConditionPageData[0].contact.subTitle.text1}
            </p>
            <p className="mb-5">
              {termsAndConditionPageData[0].contact.subTitle.text2}{" "}
              <Link className="text-primary" href={`${termsAndConditionPageData[0].contact.cta1.href}`}>
                {termsAndConditionPageData[0].contact.cta1.text}
              </Link>
            </p>
            <p className="mb-5">
             {termsAndConditionPageData[0].contact.subTitle.text3}{" "}
              <Link className="text-primary" href={`${termsAndConditionPageData[0].contact.cta2.href}`}>
                {termsAndConditionPageData[0].contact.cta2.text}
              </Link>
            </p>
          </div>
...
export async function getStaticProps({ locale }: { locale: string }) {
  let termsAndConditionPageData: TtermsAndConditionData=await getTermsAndConditionsData(locale);
  return {
    props: {
      termsAndConditionPageData
      },
  };
}